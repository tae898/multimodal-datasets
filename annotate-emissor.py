...
def get_existing_path(DATASET, modality, SPLIT, uttid, extensions):
    candidates = [
        os.path.join(DATASET, modality, SPLIT, uttid + extension)
...
class DataSet:
    def __init__(self, dataset):
        ...
        self.SPLITS = list(
            set([dir.split("/")[-1] for dir in glob(f"./{self.dataset}/raw-*/*")])
        )
        ...
        if os.path.isfile(utterance_ordered_path):
            logging.info(
                f"ordered utterances in dialgoues found " f"at {utterance_ordered_path}"
            )
            with open(utterance_ordered_path, "r") as stream:
                self.utterance_ordered = json.load(stream)
        else:
            logging.info(
                f"ordered utterances not found. Every utterance "
                f"is considered to be one dialogue."
            )
            uttids = {
                SPLIT: glob(f"./{self.dataset}/raw-texts/{SPLIT}/*.json")
    ...
    def _batch_diaids(self):
        diaids = [
            f"{SPLIT}/{diaid}"
            for SPLIT, diaids in self.utterance_ordered.items()
            for diaid in diaids
        ]
        ...
        logging.debug(f"every batch will have {BATCH_SIZE} samples (dialogues)")

        self.diaids_batched = [
            diaids[i * BATCH_SIZE : (i + 1) * BATCH_SIZE] for i in range(self.num_jobs)
        ]
...
class Emissor:
    def __init__(
        ...
        logging.debug(
            f"creating video2frames container, {self.video2frames_port} "
            f"to  {port_docker_video2frames}..."
        )
        self.container = docker.run(
            image="tae898/video2frames",
            detach=True,
                ...
                self.paths[modality] = {
                    SPLIT: {
                        uttid: get_existing_path(
                            self.dataset, modality, SPLIT, uttid, exts[modality]
                        )
    ...
    def face_recognition_dia(self, SPLIT, diaid):
        ...
        logging.debug(
            f"{diaid} has {len(self.utterance_ordered[SPLIT][diaid])} " f"utterance(s)"
        )
        for uttid in self.utterance_ordered[SPLIT][diaid]:
            try:
                ...
                assert (
                    len(face_features) == face_metadata["num_frames_original"]
            ...
            embs_utt = [[f["normed_embedding"] for f in ff] for ff in face_features]
        ...
        unique_faces_unpacked = get_unique_faces(
            embs_unpacked, self.face_cos_distance_threshold
        )
    ...
    def load_images_utt(self, SPLIT, uttid):
        ...
        if frames is not None:
            ...
            if (
                ...
                and face_path is not None
                and age_path is not None
                and gender_path is not None
            ):
                ...
                genders = [genders[idx] for idx in metadata["frame_idx_original"]]
    ...
    def annotate_write_frames(
        ...
        for idx, frame, ff in zip(frame_idx_original, frames, face_features):
            ...
            os.makedirs(
                os.path.join(self.dataset, "emissor", SPLIT, diaid, "image"),
                exist_ok=True,
            )
            ...
            frame_info = {}
            frame_info["files"] = [os.path.join("image", os.path.basename(save_path))]
            ...
            frame_info["ruler"] = {
                "bounds": [0, 0, frame.size[0], frame.size[1]],
    ...
    def write_image_emissor(self, SPLIT, diaid, image_emissor_dia):
        os.makedirs(
            os.path.join(self.dataset, "emissor", SPLIT, diaid, "image"), exist_ok=True
        )

        with open(
            os.path.join(self.dataset, "emissor", SPLIT, diaid, "image.json"), "w"
        ) as stream:
            json.dump(image_emissor_dia, stream, indent=4)

    def write_chat_emissor(self, SPLIT, diaid, chat_emissor_dia):
        os.makedirs(
            os.path.join(self.dataset, "emissor", SPLIT, diaid, "text"), exist_ok=True
        )
        with open(
            os.path.join(self.dataset, "emissor", SPLIT, diaid, "text", f"{diaid}.csv"),
            "w",
        ) as stream:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="annotate a dataset in the " "emissor format."
    )
    ...
    parser.add_argument(
        ...
        default=0.9,
        type=float,
        help="face detection probability threshold",
    )
    parser.add_argument(
        "--face-cos-distance-threshold",
        default=0.8,
        type=float,
        help="cosine distance threshold. The "
    ...
    parser.add_argument(
        "--fps-max", type=int, default=1, help="fps for the " "image modality"
    )
