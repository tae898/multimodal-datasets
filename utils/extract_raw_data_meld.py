...
def extract_text():
    ...
    for DATASET in ["train", "val", "test"]:
        ...
        for row in annotations[DATASET][1:]:
            ...
            with open(
                f"./MELD/raw-texts/{DATASET}/"
                f"dia{Dialogue_ID}_utt{Utterance_ID}.json",
                "w",
            ) as stream:
                json.dump(to_dump, stream, indent=4, sort_keys=True, ensure_ascii=False)

        utts_all = [
            utt
            for dia, diautts in utterance_ordered[DATASET].items()
            for utt in diautts
        ]

        assert (
            len(labels[DATASET])
            == len(set(labels[DATASET]))
            == len(utts_all)
            == len(set(utts_all))
        )
    ...
    with open(f"./MELD/utterance-ordered.json", "w") as stream:
        json.dump(utterance_ordered, stream, indent=4, ensure_ascii=False)
