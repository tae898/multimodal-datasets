...
def get_filepaths():
    ...
    textpaths = glob("./IEMOCAP/IEMOCAP_full_release/*/dialog/transcriptions/*.txt")
...
def move_raw_files():
    ...
    for paths, modality in tqdm(
        zip(
            [videopaths, audiopaths, textpaths],
...
def make_splits():
    ...
    for SPLIT in ["train", "val", "test"]:
        dialogxl_videoids[SPLIT] = sorted(
            list(
                set(["_".join(foo.split("_")[:-1]) for foo in dialogxl_videoids[SPLIT]])
            )
        )
...
def clean_texts():
    ...
    for foo in glob("*/*.txt"):
        ...
        for line in lines:
            ...
            try:
                ...
                with open(
                    os.path.join(SPLIT, dia, utterance_id + ".json"), "w"
                ) as stream:
    ...
    for SPLIT in ["train", "val", "test"]:
        ...
        for dia, utts in diautt_ordered[SPLIT].items():
            ...
            for utt in utts:
                path_to_check = os.path.join(f"raw-audios/{SPLIT}/{dia}/{utt}.wav")
                print(path_to_check)
    ...
    print(
        f"There are in total of {weird} weird utterances. It's fine I went "
...
def slice_videos():
    ...
    dias = {
        SPLIT: sorted(os.listdir(f"IEMOCAP/raw-videos/{SPLIT}/"))
        for SPLIT in ["train", "val", "test"]
    }

    dias = {
        SPLIT: [foo.split(".avi")[0] for foo in dias[SPLIT]]
        for SPLIT in ["train", "val", "test"]
    }
    ...
    def get_start_end_sec(audio_utt_path):
        text_utt_path = audio_utt_path.replace("raw-audios", "raw-texts").replace(
            ".wav", ".json"
        )
    ...
    def process_dia(SPLIT, dia):
        ...
        audio_dia = librosa.core.load(
            f"IEMOCAP/raw-videos/{SPLIT}/{dia}.avi", sr=SAMPLING_RATE
        )[0]

        # start_from = 0
        for utt in diautt_ordered[SPLIT][dia]:
            uttwav = os.path.join(f"IEMOCAP/raw-audios/{SPLIT}/{dia}/{utt}.wav")
            ...
            end_frame = math.ceil(
                len(video_dia) * end / (len(audio_dia) / SAMPLING_RATE)
            )
            ...
            os.makedirs(f"IEMOCAP/raw-videos/{SPLIT}/{dia}", exist_ok=True)
...
def clean_files():
    for modality in ["raw-videos", "raw-audios", "raw-texts"]:
        extension = {"raw-videos": ".mp4", "raw-audios": ".wav", "raw-texts": ".json"}[
            modality
        ]
...
def create_labels():
    ...
    text_paths = sorted(
        glob("./IEMOCAP/IEMOCAP_full_release/Session*/dialog/EmoEvaluation/Ses*.txt")
    )
    ...
    for SPLIT in ["train", "val", "test"]:
        num_utts = len([val_ for key, val in labels[SPLIT].items() for val_ in val])

        assert num_utts == len(
            [
                uttid
                for diaid, list_of_utts in utterance_ordered[SPLIT].items()
    ...
    undecided = {
        SPLIT: {
            uttid: bar
            for diaid, foo in undecided[SPLIT].items()
            for uttid, bar in foo.items()
        }
        for SPLIT in ["train", "val", "test"]
    }
    ...
    labels = {
        SPLIT: {
            diaid: {
                uttid: label_map[bar["voted"]].lower() for uttid, bar in foo.items()
            }
            for diaid, foo in labels[SPLIT].items()
        }
        for SPLIT in ["train", "val", "test"]
