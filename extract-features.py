    ...
    for video_path in tqdm(video_paths):
        try:
            ...
            for frame_bytestring, idx in zip(frames, metadata['frame_idx_original']):
                ...
                data = jsonpickle.encode(data)
                response = requests.post(
                    f"{'http://127.0.0.1'}:{face_detection_recognition_port}/", json=data)
...
class Features():
    ...
    def extract_face_features(
            self, port_docker_video2frames, port_docker_face_detection_recognition,
            port_docker_age_gender,
            width_max, height_max, fps_max, num_jobs):

        self.port_docker_video2frames = port_docker_video2frames
        self.port_docker_face_detection_recognition = port_docker_face_detection_recognition
        ...
        Parallel(n_jobs=self.num_jobs)(
            delayed(begin_face_features_extraction)(
                self.dataset, video_paths, video2frames_port,
                face_detection_recognition_port, age_gender_port,
                self.fps_max, self.width_max, self.height_max, self.video_ext)
            for video_paths, video2frames_port, face_detection_recognition_port, age_gender_port
            in zip(self.video_paths_batch, self.video2frames_ports,
                   self.face_detection_recognition_ports, self.age_gender_ports))
    ...
    def _start_face_containers(self):
        ...
        logging.info(
            f"Creating {self.num_jobs} containers of age-gender ...")
...
def main(dataset, port_docker_video2frames, port_docker_face_detection_recognition,
         port_docker_age_gender, width_max,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--port-docker-video2frames', type=int,
                        default=10001)
    parser.add_argument('--port-docker-face-detection-recognition', type=int,
                        default=10002)
    parser.add_argument('--port-docker-age-gender', type=int,
                        default=10003)
